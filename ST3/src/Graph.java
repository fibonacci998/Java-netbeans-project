...
//=================================================================
  ...
  void fbreadthDeg(int  k, RandomAccessFile f) throws Exception
   {boolean [] en = new boolean[20];
    ...
    for(i=0;i<n;i++) en[i]=false;
    fbreadthDeg(en,k,f);
    for(i=0;i<n;i++) if(!en[i]) fbreadthDeg(en,i,f);
   }
...
void checkEulerCycle(RandomAccessFile  f123) throws IOException
 {
     ...
     if (k>1) isConnected=false;
     else isConnected=true;
     calDeg();
     hasAllEvenEdge=true;
     for (int i=0;i<n;i++)
         if (deg[i]%2==1) hasAllEvenEdge=false;
     
     if (checkUndirect) f123.writeBytes("The graph is undirected.\n");
     else f123.writeBytes("The graph is directed.\n");
     if (isConnected) f123.writeBytes("The graph is connected.\n");
     else f123.writeBytes("The graph is not connected\n");
     if (hasAllEvenEdge) f123.writeBytes("All vertices have even degree.\n");
     ...
     else f123.writeBytes("Conditions for Euler's cycle are not satisfied.\n");
 }
